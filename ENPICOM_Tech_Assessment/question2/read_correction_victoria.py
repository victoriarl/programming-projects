...
def rev_comp(dna: str):
    """ Generates the reverse complement of an uppercase DNA string.
...
def find_corrections(incorrect_reads: list, correct_reads: list):
    """ CORRECTION IS HERE """
    distance_one_reads = [] # Create a list to count the number of reads at a Hamming distance 1
    corrected_reads = []
    for read_forw in incorrect_reads:
        # Find the correct read
        for correct_read in correct_reads:
            # Measure distance to a correct read
            if calc_hamm_dist(read_forw, correct_read) == 1:
                # Found a read with Hamming distance 1, add a tuple to the list of reads at a Hamming distance of 1
                distance_one_reads.append((read_forw, correct_read))
                break
            # Measure distance to the reverse of a correct read
            if calc_hamm_dist(rev_comp(read_forw), correct_read) == 1:
                # Found a read with Hamming distance 1, add a tuple to the list of reads at a Hamming distance of 1
                distance_one_reads.append((read_forw, rev_comp(correct_read)))
                break
        # Check if there was only one read with a Hamming distance 1
        if len(distance_one_reads) == 1:
            # If there was only one read at a Hamming distance 1, the incorrect read is indeed incorrect
            # add the correction to the list of all corrections
            corrected_reads += distance_one_reads
        # Empty the list of reads for the next incorrect read
        distance_one_reads = []
